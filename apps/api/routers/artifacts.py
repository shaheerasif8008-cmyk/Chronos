...
async def list_artifacts(
    ...
    """List the current version of each artifact for a conversation or task."""
    artifacts = await reflect_table("artifacts")
    async with engine.begin() as conn:
        q = select(artifacts).where(
            artifacts.c.organization_id == member.organization_id,
            # Only the latest version of each artifact, so iterated artifacts
            # don't show up as duplicates in the chat.
...
async def list_artifact_versions(
    ...
    """Return the full version history of an artifact, oldest first."""
    meta = await get_artifact(artifact_id)
    if not meta or str(meta.get("organization_id")) != str(member.organization_id):
        raise HTTPException(status_code=404, detail="Artifact not found")
...
async def create_artifact_version(
    ...
    """Save a human edit as a new version of the artifact.

    The path id identifies the artifact lineage, not a specific version: the new
    version is always appended after the current one (resolved by the artifact's
    stable key + scope), so editing from any version's id produces the next
    latest version.
    """
    meta = await get_artifact(artifact_id)
    if not meta or str(meta.get("organization_id")) != str(member.organization_id):
        raise HTTPException(status_code=404, detail="Artifact not found")
    # RULE 3: every write action routes through the permission seam.
