...
Tests:
1. _normalize_mode coerces unknown/None → "default", passes valid modes through.
2. create_task_record writes mode to the task insert row.
3. Fast-path send_message persists chosen mode on the assistant message.
"""
import pytest


# ── 1. _normalize_mode unit ───────────────────────────────────────────────────

def test_normalize_mode_none_becomes_default():
    from routers.chat import _normalize_mode
    assert _normalize_mode(None) == "default"


def test_normalize_mode_unknown_becomes_default():
    from routers.chat import _normalize_mode
    assert _normalize_mode("xyz") == "default"
