...
| Capability | Target parity | Current state | Implementation area | Interface and persistence | Acceptance proof |
|---|---|---|---|---|---|
| Artifact creation | Assistant creates durable reusable outputs from chat/tasks. | Foundation present for renderable `fs.write`. | Artifact core/router, runtime. | Artifact row, object storage/local fallback, message/task linkage. | E2E create artifact, refresh, download. |
| Artifact workspace | Side-by-side preview, full artifact browser, project/task grouping. | Missing as full workspace. | Artifacts UI/API. | Artifact collections and filters. | Playwright artifact browser, permissions, refresh. |
| Artifact editing | User and AI can edit artifacts without destructive overwrite. | Missing. | Artifact editor, version API. | `artifact_versions`, diff metadata. | Edit creates new version; restore works. |
| Artifact renderers | Markdown, HTML, React, SVG, diagrams, CSV/table, JSON, code, image, docs/sheets/slides. | Partial: mime/kind persistence; renderer coverage incomplete. | Renderer UI, sandbox. | Renderer selection and safe preview policy. | Renderer tests per supported type. |
| Artifact publish/share | Controlled share/publish/unpublish with permissions. | Missing. | Sharing API/UI, permissions. | Share links, ACL, publish status. | Unauthorized cannot access; unpublish revokes. |
