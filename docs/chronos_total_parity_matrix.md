...
|---|---|---|---|---|---|
| General chat assistant | ChatGPT/Claude-grade conversational assistant with streaming, history, model selection, citations, artifacts, memory refs, and tool traces. | Foundation present: chat route, streaming, conversations, messages, model selection, tool-capable routing. | Chat UI, chat router, LLM layer, message model. | Conversation/message records include model, mode, citations, tool traces, memory refs, artifacts, status. | API and Playwright test: send message, stream answer, refresh, history and metadata persist. |
| Model selection | User can choose models/modes by task with safe fallback and clear provider status. | Partial: chat models endpoint and selected model storage exist. | LLM config, chat composer, settings. | Model registry with label, provider, capabilities, status, policy constraints. | Test model selection persists across a tool task and fallback is visible. |
...
| Approval system | Risky writes pause, route to authorized approver, resume exactly once, reject cleanly. | Foundation present for approvals and Gmail drafts; broad policy incomplete. | Approvals router/UI, broker policy. | Approval records, payload, status, actor, resume result. | E2E: unauthorized cannot approve; authorized approval resumes task once. |
...
| Tenant isolation | Every API/query/tool/artifact/memory path enforces organization and permission scope. | Foundation present; full cross-tenant suite needed. | DB queries, routers, broker, storage. | Org/region on every persisted row and storage path. | Cross-tenant tests for memory, artifacts, tasks, connectors, projects. |
...
| Explicit memory | User says remember/forget/edit and memory updates immediately. | Partial: explicit writes exist; forget/edit UX must be verified. | Chat route, memory router/UI. | Memory entry and audit event. | E2E remember, retrieve, edit, delete. |
...
> Phase 5 proof strategy: backend behavior is proven by DB-backed pytest against an isolated database; UI is typecheck-verified via `npm run build` (it compiles and statically generates but does not render/click). Full Playwright behavioral E2E remains separate harness work (no harness in repo).
>
...
| Capability | Target parity | Current state | Implementation area | Interface and persistence | Acceptance proof |
|---|---|---|---|---|---|
| Connector directory | App catalog with setup, scopes, actions, risk, health, policy, audit. | Foundation present with OAuth catalog/framework; full product incomplete. | Connectors UI/API, repository, health. | Connector definitions, installations, policies, health. | Connect app fixture, see health/actions/policy. |
