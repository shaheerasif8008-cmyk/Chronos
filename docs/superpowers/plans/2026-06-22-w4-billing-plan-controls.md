...
- **W4.2 — Billing provider seam + subscription lifecycle (DONE, seam only).** `core/billing.py` provider seam (Stripe behind config, truthful-degraded otherwise); checkout/portal links; webhook → `organizations.plan` sync; audited. _Live-Stripe-wiring follow-ups (deferred until the SDK is wired; pinned in `_parse_event`'s SECURITY CONTRACT comment): derive event `org_id`/`plan` from server-bound checkout metadata + the price→plan map (never the raw payload); handle `customer.subscription.deleted` (downgrade); widen the webhook except to catch `SignatureVerificationError`→400; guard an unmapped price (don't silently downgrade to trial)._
