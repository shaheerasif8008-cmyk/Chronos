...
- **Phase 2B — Org-bound token flip + handoff + C1/C2 + prod signup.** Flip all mint sites (`/auth/signup`, `verify_otp`, `cognito/callback`, `cognito/verify`, `sso.py`) to `create_access_token(member_id, org_id=...)`. Add `POST /auth/handoff` issuing/consuming the short-lived cross-subdomain handoff token (spec §2e). Enforce **C1**: reject org-bound tokens when `resolved_org_id is None` *except* the handoff endpoint. Add `settings.enforce_org_bound_tokens`; when on, `get_current_member` rejects org-less tokens (**C2** — closes grandfathering). Migrate the test harness to subdomain base URLs / `X-Chronos-Org`. This is the phase that makes tenancy *live*.
  ...
  - **Self-serve login path + free-email idempotency (from 2A final review — REQUIRED before opening to real users).** Two coupled gaps, both inert behind 2A's dev-OTP-only endpoint: (1) `verify_otp` → `get_member_by_email` filters on `organization_id == settings.org_id` (`"default"`), so a member of a self-serve-created org **cannot log in** via `verify_otp` — login must resolve the member's actual org (by subdomain/tenant context once binding is live, or a global email→member lookup). (2) The free-email branch in `signup_or_join` is **non-idempotent**: a returning free-email user gets a brand-new personal org each time, orphaning the prior one (the work-domain branch is idempotent only because re-signup resolves the existing claim → existing member). 2B must add a real login path for self-serve orgs and an existing-personal-org lookup so free-email re-signup logs in instead of duplicating. Add tests for repeat free-email signup and self-serve member login.
  - **Pending-member re-signup mints a dead token (inert in 2A).** A `pending_approval` member who re-submits hits the `existing is not None` branch (no `status` key returned) → the endpoint mints a token, but `get_current_member` 403s on non-active status, so it's dead-on-arrival (not an access hole). Unreachable in 2A (no code sets `join_policy="approval"`). Handle in the 2B approval-inbox work: return a pending status instead of a dead token.
  - **TOCTOU race hardening (from 2A Chunk-2 review — REQUIRED before opening the endpoint).** `signup_or_join`'s unclaimed-work-domain branch does `unique_subdomain` (read) → `provision_org` (write) → claim insert across separate transactions. Two concurrent same-domain signups can (a) collide on `uq_organizations_subdomain` or `uq_email_domain_claims_domain` and surface an unhandled 500, and (b) in the differing-suffix case, leave an **orphan org** (org+owner committed, no claim points to it). DB constraints prevent corruption, but 2B must wrap the branch in an `IntegrityError` catch that re-resolves to auto-join (re-read the now-existing claim → `provision_member`) and cleans up / avoids the orphan (e.g. claim-first as the atomic gate, or roll back the org on claim collision). Add a monkeypatch-based test that forces the collision path and asserts a clean auto-join, not a 500. Inert in 2A (dev-OTP-only endpoint).
